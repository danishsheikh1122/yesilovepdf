...
// This component avoids canvas node dependencies by using PDF.js only in browser
export default function CropPdfBrowser() {
    ...
    if (typeof window !== 'undefined' && !pdfjsLoaded) {
      ...
      script.onload = async () => {
        try {
          ...
          // Fallback - we'll just allow file upload and process on backend
          setPdfjsLoaded(true);
        }
      };
    ...
    try {
      if (pdfjsLoaded && typeof window !== 'undefined') {
        ...
        try {
          ...
          console.warn('PDF.js loading failed, using fallback:', pdfError);
          // Fallback: just store the file and show a basic interface
          setPdfFile(file);
          setTotalPages(1); // We'll assume at least 1 page
          setCurrentPage(1);
          setCropArea(null);
        }
      } else {
        // Fallback mode - just store the file
        setPdfFile(file);
        setTotalPages(1);
        setCurrentPage(1);
        setCropArea(null);
  ...
  // Render PDF page to canvas (only if PDF.js is available)
  ...
  const handleCropPdf = async () => {
    ...
    try {
      // Calculate scaling based on whether we have canvas or not
      let scaleX = 1;
      let scaleY = 1;
                ...
                >
                  ...
                  <div className="flex flex-col items-center space-y-3">
                    <Button 
                      ...
                      onClick={() => document.getElementById('crop-file-upload')?.click()}
                    >
                      Select PDF files
                    </Button>
                ...
                <div className="space-y-4">
                  ...
                  >
                    {pdfDoc ? (
                      ...
                      <div className="w-full h-64 bg-gray-100 flex items-center justify-center cursor-crosshair">
                        <div className="text-center">
                          <p className="text-gray-500 mb-2">PDF Preview</p>
                    ...
                    {cropArea && (
                      <>
                        ...
                        {/* Transparent crop window border (in front) */}
                        <div
                        ...
                        <div
                          ...
                          style={{ left: `${cropArea.x - 6}px`, top: `${cropArea.y - 6}px` }}
                        />
                        {/* TR */}
                        <div
                          className="absolute w-4 h-4 bg-white border-2 border-blue-400 rounded-full shadow"
                          style={{ left: `${cropArea.x + cropArea.width - 6}px`, top: `${cropArea.y - 6}px` }}
                        />
                        {/* BL */}
                        <div
                          className="absolute w-4 h-4 bg-white border-2 border-blue-400 rounded-full shadow"
                          style={{ left: `${cropArea.x - 6}px`, top: `${cropArea.y + cropArea.height - 6}px` }}
                        />
                        {/* BR */}
                        <div
                          className="absolute w-4 h-4 bg-white border-2 border-blue-400 rounded-full shadow"
                          style={{ left: `${cropArea.x + cropArea.width - 6}px`, top: `${cropArea.y + cropArea.height - 6}px` }}
                        />
                      </>
                    )}
                  </div>
