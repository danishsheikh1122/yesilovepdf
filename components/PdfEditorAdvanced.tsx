...
const PdfEditorAdvanced = ({ url, onSave }: PdfEditorAdvancedProps) => {
  ...
  // Editor state
  const [elements, setElements] = useState<Map<number, EditorElement[]>>(new Map());
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [currentTool, setCurrentTool] = useState<'select' | 'text' | 'rect' | 'circle' | 'line' | 'arrow' | 'highlight'>('select');
  ...
  // Save to history
  const saveToHistory = useCallback((newElements: Map<number, EditorElement[]>) => {
  ...
  const updateElement = useCallback((id: string, updates: Partial<EditorElement>) => {
    ...
    if (elementIndex !== -1) {
      pageElements[elementIndex] = { ...pageElements[elementIndex], ...updates };
      newElements.set(currentPage, pageElements);
      setElements(newElements);
      saveToHistory(newElements);
    }
  ...
  }, [currentTool, fontSize, fontFamily, fontStyle, textDecoration, textColor, opacity, addElement]);

  // Handle stage mouse move
  const handleMouseMove = useCallback((e: KonvaEventObject<MouseEvent>) => {
    if (!isDrawing || !drawStart || currentTool === 'select' || currentTool === 'text') return;
  ...
  const handleMouseUp = useCallback((e: KonvaEventObject<MouseEvent>) => {
    ...
    setDrawStart(null);
    setCurrentTool('select');
  }, [isDrawing, drawStart, currentTool, fillColor, strokeColor, strokeWidth, fontSize, textColor, opacity, addElement]);

  // Handle text double-click for inline editing
  const handleTextDblClick = useCallback((element: EditorElement) => {
    const textNode = stageRefs.current.get(currentPage)?.findOne(`#${element.id}`);
    if (!textNode) return;
    ...
    const commonProps = {
      ...
      onTransformEnd: (e: KonvaEventObject<Event>) => {
        ...
        updateElement(element.id, {
          ...
          height: Math.max(5, (node.height && typeof node.height === 'function' ? node.height() : 0) * scaleY),
        });
      },
    };
  ...
  return {
    ...
    // Render function
    renderViewer: () => (
      <div style={{ position: 'relative', width: pageSize.width, height: pageSize.height }}>
        {canvas && (
          <canvas
