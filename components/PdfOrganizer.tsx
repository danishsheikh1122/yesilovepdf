...
export default function PdfOrganizer({
  ...
  const loadPageThumbnail = async (pageNumber: number) => {
    try {
      ...
      const existingThumbnail = thumbnails.find(
  ...
  const handleDrop = (
    ...
    setOrganizedPages((prev) => {
      const newPages = [...prev];
      const draggedIndex = newPages.findIndex(
        (page) => page.id === draggedItem
      );
  ...
  const handleOrganize = async () => {
    ...
    // Track the organize action
    await pdfTrackers.organize(file);
  };
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-sm text-gray-600">
              {organizedPages.length} page
              {organizedPages.length !== 1 ? "s" : ""}
            </div>
          </div>

          <div className="text-sm text-gray-600 mb-4">
            Drag pages to reorder • Click rotate to turn pages • Remove unwanted
            pages
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {organizedPages.map((page, index) => {
            const thumbnail = thumbnails.find(
              (t) => t.pageNumber === page.pageInfo.pageNumber
            );
        ...
        {organizedPages.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-gray-500 mt-1">
              Upload a new file to start organizing
            </p>
          </div>
        )}
      </CardContent>
