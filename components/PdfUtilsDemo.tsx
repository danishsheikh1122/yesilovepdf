...
export default function PdfUtilsDemo() {
  ...
  const handleCreatePdf = async () => {
    ...
    try {
      ...
      const pdfDoc = await createPdf({
        ...
        fontUrl: '/fonts/default.ttf', // Add required fontUrl property
        pages: [
          {
            text: formData.text,
      ...
      // Track the PDF creation
      await pdfTrackers.edit("created-pdf.pdf");
    } catch (error) {
    ...
    try {
      ...
      switch (action) {
        ...
        case "modify":
          ...
          const modifiedDataUrl = (await exportPdf(
            result,
            "dataurl"
          )) as string;
  ...
  const handleMergeFiles = async (
    ...
    try {
      ...
      await pdfTrackers.merge(Array.from(files));
    } catch (error) {
      log("❌ Error: " + (error as Error).message);
    }
    setLoading(false);
  ...
  const demoFunctions = [
    ...
    },
    {
      ...
      fileInput: true,
      fileAction: (e: React.ChangeEvent<HTMLInputElement>) =>
        handleFileUpload(e, "info"),
      color: "bg-green-500",
    },
    {
      title: "Read Metadata",
    ...
    },
    {
      ...
      fileInput: true,
      fileAction: (e: React.ChangeEvent<HTMLInputElement>) =>
        handleFileUpload(e, "addMetadata"),
      color: "bg-indigo-500",
    },
    {
      title: "Merge PDFs",
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="text-center space-y-4">
          <h1 className="text-4xl font-bold text-gray-900">
            PDF-LIB Utility Demo
          </h1>
          <p className="text-gray-600 max-w-2xl mx-auto">
            Complete PDF manipulation system using PDF-LIB. Create, modify,
            merge, and export PDFs with advanced features.
          </p>
        </div>
                ...
                <Card key={index} className="hover:shadow-lg transition-shadow">
                  <CardContent className="p-4">
                    <div className="flex items-start gap-4">
                      <div
                        className={`p-3 rounded-lg ${func.color} text-white`}
                      >
                        {func.icon}
                      </div>
                        ...
                        {func.fileInput ? (
                          <div className="space-y-2">
                            <input
                              ...
                              multiple={
                                "multiple" in func ? func.multiple : false
                              }
                              onChange={
                                "fileAction" in func
                                  ? func.fileAction
